...
final class SchemaEnforcer {
	...
	public function enforce( array $request ) {
		...
		// Hidden fields carry a value configured by the form author. Never let
		// a visitor replace it with a forged request value.
		foreach ( $schema as $slug => $field ) {
			if ( 'hidden' === $field['type'] ) {
				$submitted[ $slug ] = (string) ( $field['attributes']['value'] ?? '' );
			}
		}
