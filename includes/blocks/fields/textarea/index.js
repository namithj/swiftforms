...
const DEFAULT_SLUG = 'message';

registerBlockType( 'swiftforms/textarea-field', {
    edit( { attributes, setAttributes, clientId } ) {
        const { helpText, label, placeholder, required, slug } = attributes;
        const blockProps = useBlockProps( { className: 'swiftforms-field swiftforms-field--textarea' } );
        ...
        return (
            <div { ...blockProps }>
                <InspectorControls>
                    <PanelBody title={ __( 'Textarea Settings', 'swiftforms' ) } initialOpen={ true }>
                ...
                <RichText tagName="span" className="swiftforms-field__label" value={ label } onChange={ onLabelChange } placeholder={ __( 'Textarea label', 'swiftforms' ) } />
                <textarea disabled placeholder={ placeholder || __( 'Tell us more...', 'swiftforms' ) } rows="4"></textarea>
    ...
    save( { attributes } ) {
        ...
        const blockProps = useBlockProps.save( {
            className: 'swiftforms-field swiftforms-field--textarea',
            'data-field-slug': slug || 'message',
        ...
        return (
            <div { ...blockProps }>
                <label className="swiftforms-field__control">
                    ...
                    <textarea name={ slug || 'message' } placeholder={ placeholder || '' } required={ required } rows="4"></textarea>
                </label>
                { helpText ? <p className="swiftforms-field__help">{ helpText }</p> : null }
            </div>
        );
