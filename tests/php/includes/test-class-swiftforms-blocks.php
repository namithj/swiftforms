...
class SwiftForms_Blocks_Test extends WP_UnitTestCase {
	...
	public function test_enqueue_frontend_settings_attaches_runtime_data(): void {
		...
		$scripts       = wp_scripts();
		$inline_script = implode( "\n", $scripts->get_data( 'swiftforms-form-view-script', 'before' ) ? $scripts->get_data( 'swiftforms-form-view-script', 'before' ) : array() );
	...
	public function test_render_form_block_does_not_leak_notification_settings_into_markup(): void {
		// Admin recipients/templates must never reach the public HTML: they're
		// never read by the frontend script and would otherwise let any visitor
		// read the admin's inbox address and email templates from page source.
		$blocks = new SwiftForms_Blocks( SWIFTFORMS_PATH );
		$blocks->register_blocks();
		...
		$this->assertStringNotContainsString( 'ops@example.com', $markup );
		$this->assertStringNotContainsString( 'Secret template', $markup );
		$this->assertStringNotContainsString( 'data-admin-recipients', $markup );
		$this->assertStringNotContainsString( 'data-admin-template', $markup );
	}
	...
	public function test_render_form_block_injects_conditions_data_attribute(): void {
		$blocks = new SwiftForms_Blocks( SWIFTFORMS_PATH );
		$blocks->register_blocks();

		$form_id = self::factory()->post->create(
			array(
				'post_content' => '<!-- wp:swiftforms/text-field {"label":"Details","slug":"details","conditions":{"enabled":true,"action":"show","groups":[[{"field":"topic","operator":"equals","value":"support"}]]}} --><div class="wp-block-swiftforms-text-field swiftforms-field swiftforms-field--text" data-field-slug="details" data-field-type="text" data-swiftforms-field="true"><label class="swiftforms-field__control"><span class="swiftforms-field__label">Details</span><input name="details" type="text" /></label></div><!-- /wp:swiftforms/text-field -->',
				'post_status'  => 'publish',
				'post_type'    => SwiftForms_CPTs::FORM_POST_TYPE,
			)
		);
		...
		$this->assertStringContainsString( 'data-sf-conditions=', $markup );
		$this->assertStringContainsString( '&quot;operator&quot;:&quot;equals&quot;', $markup );
	}

	public function test_render_form_block_omits_conditions_attribute_without_rules(): void {
		$blocks = new SwiftForms_Blocks( SWIFTFORMS_PATH );
		$blocks->register_blocks();

		$form_id = self::factory()->post->create(
			array(
				'post_content' => '<!-- wp:swiftforms/text-field {"label":"Details","slug":"details"} --><div class="wp-block-swiftforms-text-field swiftforms-field swiftforms-field--text" data-field-slug="details" data-field-type="text" data-swiftforms-field="true"><label class="swiftforms-field__control"><span class="swiftforms-field__label">Details</span><input name="details" type="text" /></label></div><!-- /wp:swiftforms/text-field -->',
				'post_status'  => 'publish',
				'post_type'    => SwiftForms_CPTs::FORM_POST_TYPE,
			)
		);
		...
		$this->assertStringNotContainsString( 'data-sf-conditions', $markup );
	}

	public function test_register_blocks_registers_round_b_blocks(): void {
		$blocks = new SwiftForms_Blocks( SWIFTFORMS_PATH );

		$blocks->register_blocks();

		$this->assertNotNull( WP_Block_Type_Registry::get_instance()->get_registered( 'swiftforms/radio-field' ) );
		$this->assertNotNull( WP_Block_Type_Registry::get_instance()->get_registered( 'swiftforms/date-field' ) );
		$this->assertNotNull( WP_Block_Type_Registry::get_instance()->get_registered( 'swiftforms/hidden-field' ) );
		$this->assertNotNull( WP_Block_Type_Registry::get_instance()->get_registered( 'swiftforms/step' ) );
	}

	public function test_filter_allowed_block_types_allows_step_in_form_posts_only(): void {
		$blocks  = new SwiftForms_Blocks( SWIFTFORMS_PATH );
		$form_id = self::factory()->post->create( array( 'post_type' => SwiftForms_CPTs::FORM_POST_TYPE ) );
		$page_id = self::factory()->post->create( array( 'post_type' => 'page' ) );

		$form_allowed = $blocks->filter_allowed_block_types(
			array( 'swiftforms/step', 'swiftforms/radio-field', 'core/paragraph' ),
			new WP_Block_Editor_Context( array( 'post' => get_post( $form_id ) ) )
		);
		$page_allowed = $blocks->filter_allowed_block_types(
			array( 'swiftforms/step', 'swiftforms/radio-field', 'core/paragraph' ),
			new WP_Block_Editor_Context( array( 'post' => get_post( $page_id ) ) )
		);

		$this->assertContains( 'swiftforms/step', $form_allowed );
		$this->assertContains( 'swiftforms/radio-field', $form_allowed );
		$this->assertNotContains( 'swiftforms/radio-field', $page_allowed );
	}
}
