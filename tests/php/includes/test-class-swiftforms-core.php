...
class SwiftForms_Core_Test extends WP_UnitTestCase {
	...
	public function test_init_registers_rest_submit_route(): void {
		$core = new SwiftForms_Core();
		$core->init();

		$routes = rest_get_server()->get_routes();

		$this->assertArrayHasKey( '/swiftforms/v1/submit', $routes );
	}

	public function test_rest_submit_route_rejects_invalid_nonce_with_400(): void {
		$core = new SwiftForms_Core();
		$core->init();
		...
		$request  = new WP_REST_Request( 'POST', '/swiftforms/v1/submit' );
		...
		// The refreshed nonce must ride along so cached pages can retry.
