...
if [[ -n "$(git -C "${root_dir}" status --porcelain)" ]]; then
	echo "Commit or stash release changes before packaging." >&2
	exit 1
fi
...
( cd "${stage_dir}" && LC_ALL=C find swiftforms -print | LC_ALL=C sort | zip -X -q "${archive_path}" -@ )
